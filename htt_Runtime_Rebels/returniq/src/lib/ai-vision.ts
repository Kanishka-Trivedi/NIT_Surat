...
// ─── Analyze product VIDEO (multi-frame) with GPT-4 Vision ───────────
export async function analyzeProductVideo(
    ...
    // If no API key or empty frames, use enhanced simulation
    ...
    try {
        const content: any[] = [
            {
                type: 'text',
                text: `Analyze these ${frames.length} frames extracted from a product video return request.
Product: ${productName}
Category: ${category}

Check for consistency across frames. Look for movement or angles that verify the item's condition real-time.

Return a JSON object with these exact fields:
{
  "authentic": true/false,
  "condition": "like_new" | "good" | "fair" | "poor",
  "tags_attached": true/false,
  "damage_detected": true/false,
  "damage_description": "string or null",
  "return_eligible": true/false,
  "confidence": 0.0 to 1.0
...
Return ONLY the JSON.`,
        ...
        return {
            ...
            tags_attached: parsed.tags_attached ?? false,
            damage_detected: parsed.damage_detected ?? false,
            damage_description: parsed.damage_description ? `Video Analysis: ${parsed.damage_description}` : null,
            return_eligible: parsed.return_eligible ?? true,
            confidence: (parsed.confidence ?? 0.8) + 0.1, // Higher confidence for video
        };
    } catch (error) {
        console.error('GPT-4V video analysis failed, using simulation:', error);
        return simulatePhotoAnalysis(productName, category);
    }
}
