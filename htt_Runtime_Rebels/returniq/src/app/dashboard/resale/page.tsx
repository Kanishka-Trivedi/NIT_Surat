...
import { BrandSession, ResaleItem, SustainabilityMetrics } from '@/types';
...
export default function ResaleDashboardPage() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const res = await fetch(`/api/resale?brandId=${s.brand.id}`);
    ...
    if (loading || !session) return (
        <div className="loading-overlay" style={{ minHeight: '100vh' }}>
            ...
            <p className="loading-text">Loading resale pipeline...</p>
        </div>
    );
                        ...
                        <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '800px', padding: 0, overflow: 'hidden' }}>
                            <div style={{ background: '#111827', padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: 'white' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                    <div style={{ fontWeight: 700 }}>{session.brand.name} <span style={{ fontWeight: 400, opacity: 0.8 }}>Pre-Loved</span></div>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', minHeight: '400px' }}>
                                ...
                                <div style={{ padding: '32px' }}>
                                    ...
                                    <button className="btn btn-primary btn-lg" style={{ width: '100%', marginBottom: '12px', background: session.brand.accent_color }}>
                                        Add to Cart
                                    </button>
